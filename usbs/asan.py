...
def push_poison(inst):
    ...
    ldr r2, [sp, #-96]
  """
    return asm(calltemp, 0x08100000)
...
def push_it_poison(inst, regnum, it_cond):
    ...
    ldr r2, [sp, #-128]
  """
    return asm(calltemp % (it_cond, it_cond, regnum * 4, it_cond), 0x08100000)
...
def ret_poison(inst, regnum):
    ...
    ldr r2, [sp, #-96]
  """
    return asm(rettemp % (regnum * 4), 0x08100000)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 2:
        ...
        print(
            "Error: must pass executable filename.\nCorrect usage: %s <filename>"
            % sys.argv[0]
        )
